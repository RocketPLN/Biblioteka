...
const SearchBar = () => {
  ...
  return (
    <div className="relative w-[calc(100vw/4)] rounded-md bg-muted">
      ...
      {search.length > 0 && (
        <div className="absolute z-10 flex h-fit w-full flex-col gap-1 rounded-sm border bg-muted px-2 py-3">
          {filteredBoooks?.length ? (
            filteredBoooks?.map((book) => (
              <SearchItem book={book} key={book.id} />
            ))
          ) : (
            <span className="rounded-sm border-b bg-background/80 p-2 text-center text-lg font-bold capitalize shadow-sm">
              Nie znaleziono
            </span>
          )}
        </div>
...
const SearchItem = ({
  ...
  return (
    <div className="flex items-center gap-2 rounded-sm border-b bg-background/80 p-2 capitalize shadow-sm">
